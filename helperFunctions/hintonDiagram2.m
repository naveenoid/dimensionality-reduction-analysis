function hintonDiagram2(w, faceCol, max_m,min_m)
...
for i=1:S
  for j=1:R
    ...
    %m = sqrt((abs(w(i,j)))/max_m);
    %m = min(m,max_m)*0.95;
    if real(m)
      if w(i,j) >= 0
        h=fill(xn*m+j,yn*m+i,[0 0 0]);
        set(h,'edgecolor',col,'LineWidth',3.0);
        plot(xn1*m+j,yn1*m+i,'w',xn2*m+j,yn2*m+i,'k')
      elseif w(i,j) < 0
        h=fill(xn*m+j,yn*m+i,[1 1 1]);
        set(h,'edgecolor',col,'LineWidth',3.0);
        %plot(xn1*m+j,yn1*m+i,'k',xn2*m+j,yn2*m+i,'w');
        xn1+j,yn1+i
        xn2+j,yn2+i
        plot(xn1*m+j,yn1*m+i,'k',xn2*m+j,yn2*m+i,'k');
      end
    end
  end
end
...
%xticks(find(xticks == floor(xticks))))
...
%get(gca,'xticks')
%set(gca, 'xticks', 'interpreter', 'tex');

...
% Naveen Kuppuswamy (2013) - naveenoid@ifi.uzh.ch
   ...
   % tspanFull = linspace(tspan(1),tspan(2), 500);
    ...
    c(1:2) = norm(x(end,1:2) - desiredTheta);% - 1e-6;
    c(3) = abs(x(end,3));% - 1e-4; %FINAL velocity joint1
    c(4) = abs(x(end,4));% - 1e-4; %FINAL velocity joint2
 %   c(5) = abs(thetaDotEnd(3)) - 1e-2;%75;%FINAL acceleration joint1
 %   c(6) = abs(thetaDotEnd(4)) - 1e-2;%075;%FINAL acceleration joint2
    c(5) = abs(thetaDotIni(1));% - 1e-4; % INITIAL velocity joint1
    c(6) = abs(thetaDotIni(2));% - 1e-4; % INITIAL velocity joint2

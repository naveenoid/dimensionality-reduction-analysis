function [ result ] = testOptimalSolutionEllipse( WOpt, WIni, xAmp, yAmp,redThreshold,  tspan, mechSystem, mechOpt, basisFunc, basisOpt, inCol )
...
trajectoryName = {'Sinusoidal','Optimal', 'Intrinsic'};
clear result;
result= struct();
...
for i = 1:2
    ...
    [t, x] = ode15s(sys,10*tspan,xi,vopt);
...
legend(trajectoryName{1:2},'Location','SouthEast');%subplot(2,1,2); legend(trajectoryName{1:2},'Location','SouthEast');
grid on;
figure(figN+1); legend(trajectoryName{1:end},'Location','SouthEast'); 

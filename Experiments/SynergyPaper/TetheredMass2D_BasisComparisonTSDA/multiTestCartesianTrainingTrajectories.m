...
for tspanMax = tspanRange
 ...
 %   [score1, redOrder , redSys, hsvs, normHsvs ] = evaluateDimCost_BT(ones(mechOpt.inputDim),mechOpt,tempOpt,redThreshold,[],[],[],1);
    ...
    %polyBasis setup;
%     basisFunc{1} = @polyBasisNormalised;
%     basisLearning{1} = @polyBasisWeightLearningNormalised;
%     basisOpt{1}.order = 6;%12; 
...
%     %polyBasis setup;
%     basisFunc{2} = @polyBasisNormalised;
%     basisLearning{2} = @polyBasisWeightLearningNormalised;
%     basisOpt{2}.order = 7;%12; 
%     basisOpt{2}.tmax = tspanMax;
%     inputDim{2} = basisOpt{1}.order+1;

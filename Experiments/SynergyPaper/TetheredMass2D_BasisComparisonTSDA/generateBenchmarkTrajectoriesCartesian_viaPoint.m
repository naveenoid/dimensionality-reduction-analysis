function [ uFunc ] = generateBenchmarkTrajectoriesCartesian( xStart, xMid, xEnd,tspan, opt,cols )
...
%     end
  ...
  % spy(:,2:numPoints+2) = iKin(spyC(:,2:numPoints+2)','down',opt)';
   ...
   % spline1 = csape(spxNew, [0 spyCNew(1,:) 0],[1 1]);
   spline1 = csape(spx,spyC(1,:));
   % spline2 = csape(spxNew, [0 spyCNew(2,:) 0],[1 1]);
   %

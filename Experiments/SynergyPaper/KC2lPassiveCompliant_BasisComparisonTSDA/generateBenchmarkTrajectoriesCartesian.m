function [ uFunc ] = generateBenchmarkTrajectoriesCartesian( xEnd, tspan,trajectoryType, opt,cols )
   ...
   xStart = pts(3:4)';
    ...
    switch(trajectoryType)
        ...
        case 2
            ...
            spyC(:,2) = [0.5; 1.5].*(xEnd + xStart);%[0.5;0.47]   
        case 3 
%              numPoints = 2;
%              spy=zeros(2,numPoints+2);
%              spyC=spy;
%              fprintf('S Shaped Curve');
%               spyC(:,2) = xStart +  [0.3;0.65].*(xEnd - xStart); %[0.3;0.55].*(xEnd - xStart);                        
%               spyC(:,3) = xStart + [0.7;0.35].*(xEnd - xStart); %[0.6;0.55].*(xEnd - xStart);   
%               spy(:,3);
             numPoints = 4;
             spy=zeros(2,numPoints+2);
             spyC=spy;
             fprintf('S Shaped Curve');
              spyC(:,2) = xStart +  [0.1;0.65].*(xEnd - xStart); %[0.3;0.55].*(xEnd - xStart);                        
              spyC(:,3) = xStart + [0.3;0.65].*(xEnd - xStart); %[0.6;0.55].*(xEnd - xStart);   
              spyC(:,4) = xStart +  [0.7;0.35].*(xEnd - xStart); %[0.3;0.55].*(xEnd - xStart);                        
              spyC(:,5) = xStart + [0.9;0.35].*(xEnd - xStart); %[0.6;0.55].*(xEnd - xStart); 
              spy(:,5);
        case 4                         
            numPoints = 2;
            spy=zeros(2,numPoints+2);
            ...
            spyC(:,2) = xStart + [0.25;-0.5].*(xEnd - xStart);%[0.2;0.4].*(xEnd - xStart);%[0.33;0.37].*(xEnd - xStart);
            spyC(:,3) = xStart + [1.25;0.5].*(xEnd - xStart);%[0.8;0.9].*(xEnd - xStart);%[0.66;0.57].*(xEnd - xStart);
%         case 5          
%             tspanl = linspace(tspan(1),tspan(2),3);
%             numPoints = length(tspanl);
%             spy=zeros(2,numPoints);
...
%             numPoints = numPoints-2;
  ...
  % spy(:,2:numPoints+2) = iKin(spyC(:,2:numPoints+2)','down',opt)';
   ...
   % disp(spy);
    ...
    %spxNew = [spx(1) 0.075*spx(end) spx(2:end-1) 0.925*spx(end) spx(end)]; 
    %spxNew = [spx(1) 0.125*spx(end) spx(2:end-1) 0.875*spx(end) spx(end)]; 
    spxNew = [spx(1) 0.1*spx(end) spx(2:end-1) 0.9*spx(end) spx(end)]; 
  %  spxNew = [spx(1) 0.06*spx(end) spx(2:end-1) 0.94*spx(end) spx(end)]; 
    spyCNew = [spyC(1:2,1) spyC spyC(1:2,end)];
    
    spline1 = csape(spxNew, [0 spyCNew(1,:) 0],[2 2]);%spline(spx,spyC(1,:));
    spline2 = csape(spxNew, [0 spyCNew(2,:) 0],[2 2]);%spline(spx,spyC(2,:));

%% Optimising the dimensionality of a Legendre Basis System to reach a point in space


basisOrderRange = 4:1:6;%4:1:6;%4:1:5%5:1:6;%4:1:6;%:5;%4:1:7;%5:1:8;
tspanRange = 0.8:0.2:1.2;% 17.5:2.5:20;%10:2.5:15;%10:2.5:15;%5:0.5:7;%:5:40;%4;%3.0:0.5:4.5;%1:0.5:3;%8
endPointRange = 0.3:0.1:0.50;%0.6:0.2:1.0;%0.1:0.1:0.5;
exitFlagMat = zeros(length(endPointRange),length(basisOrderRange));
...
for endPoint = endPointRange
    ...
    %   clearvars -except tspanMax tspanRange
        ...
        % Robot setup
            ...
            initialTrajectoryType = 2;%2; % 1-step, 2-sigmoid
            ...
            basisOpt.order = basisOrder;
            basisOpt.tmax = tspanMax;
...
%
       ...
       %     toc
            ...
            if(initialTrajectoryType ==1)
                save(sprintf('./Data/SynergyPaper/TetheredMass2D_LegendreBasisMDC/MDC_StepStart_tMax_%d_basisorder_%d_target_(%d,%d)',floor(10*tspan(2)),floor(basisOpt.order),floor(10*xDes(1)),floor(10*xDes(2))),'result','WOpt' ,'WIni', 'redThreshold', 'tspan', 'mechSystem', 'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
            else
                save(sprintf('./Data/SynergyPaper/TetheredMass2D_LegendreBasisMDC/MDC_SigmoidStart_tMax_%d_basisorder_%d_target_(%d,%d)',floor(10*tspan(2)),floor(basisOpt.order),floor(10*xDes(1)),floor(10*xDes(2))),'result','WOpt' ,'WIni', 'redThreshold', 'tspan', 'mechSystem', 'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
            end
...
resultMat = zeros(size(exitFlagMat,1)+1,size(exitFlagMat,2)+1);
resultMat(2:end,2:end) = exitFlagMat;
resultMat(1,2:end) = basisOrderRange;%endpointRange;
resultMat(2:end,1) = endPointRange;
disp(resultMat);
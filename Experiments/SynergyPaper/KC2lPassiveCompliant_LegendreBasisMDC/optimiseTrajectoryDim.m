...
%[0.6,0.0]',[0.5,0.1]',[0.6,0.4]',[0.6,0.2]',[0.5,0.3]',[0.7,0.2]', [0.7,0.3]'
%xDRange = {[0.4,0.6]', [0.2,0.4]'};
% xDRange = { 
%     [0.6852,    0.2772]',...
%     [0.6237,    0.2427]',...
%     [0.5719,    0.1948]',...
...
tspanRange = 2.5:0.5:4.0;%1.5:0.5:2.0;%1.5:0.5:2.0;%2.5:0.5:5.5;%5.25;%1.0:0.5:3.5;
...
%         
        ...
        trainingFunc = generateBenchmarkTrajectoriesCartesian(desiredPos, tspan,1, mechOpt,cols(i+3*(j-1),:));
       ...
       % subplot(2,1,2);
        hold on;
        %func = basisOpt{j};
        plot(tTrain,uD','LineWidth',2.0,'color',cols(i+3*(j-1),:));
        plot(tTrain,WHat*basisFunc(tTrain, basisOpt),'--','LineWidth',2.0,'color',cols(i+3*(j-1),:)); axis tight; grid on;
        xlabel('time (sec)');
        ylabel('LegendreBasis u');
...
% 
      %  ExitFlag = 2; WOpt = WIni;
        ...
        if(mod(tspan(2),1) == 0)
            save(sprintf('./Data/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDCReaching_tSpan_%d_target_%d_%d_down_order%d',floor(tspan(2)),10*desiredPos(1),10*desiredPos(2), basisOpt.order),'result','WOpt' ,'WIni', 'redThreshold','cm', 'tspan', 'mechSystem', 'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
        else
            save(sprintf('./Data/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDCReaching_tSpan_%d_5_target_%d_%d_down_order%d',floor(tspan(2)),10*desiredPos(1),10*desiredPos(2), basisOpt.order),'result','WOpt' ,'WIni', 'redThreshold','cm', 'tspan', 'mechSystem', 'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
        end
           % save(sprintf(
    end
end

save(sprintf('./Data/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDCReachingSummary2'),'exitFlagStore','tspanRange', 'xDRange');

%Nice Lines : 
%xDRange = {[0.6,0.0]',[0.5,0.1]',[0.6,0.4]',[0.6,0.2]',[0.5,0.3]',[0.7,0.2]', [0.7,0.3]'};%{[0.7,0.2]',[0.7,0.3]',[0.6,0.0]',[0.6,0.4]',[0.6,0.2]',[0.5,0.1]'};%,[0.5,0.3]',[0.4,0.0]',[0.4,0.6]',[0.2,0.4]'};
%tDRange = {3.0, 3.0, 3.0, 3.0, 3.0,2.5, 2.5};%{2.0,  1.5, 2.0, 2.0,  1.5,2.5,2.5};

xDRange = {[0.7,0.2]',[0.6,0.4]',[0.6,0.2]', [0.6,0.0]', [0.5,0.1]', [0.4,0.2]', [0.7,0.3]'};
tDRange = {2.5       ,3.0       ,3.0       ,3.0        ,2.5        ,4.0        ,2.5};


compWith = 1;%1 - Acceleration, 2 - Jerk
        ...
        % Data for 0.3, -0.7 reaching
         if(mod(tDRange{j},1) == 0)
            try
                ...
                load(sprintf('./Data/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDCReaching_tSpan_%d_target_%d_%d_down_order5.mat',...         
                floor(tDRange{j}),10*xd(1),10*xd(2)),...
                'result','WOpt' ,'WIni', 'redThreshold','cm', 'tspan', 'mechSystem',...
                'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
            catch
                fprintf('File does not exist : MDCResult_HSV2_tSpan_%d_target_%d_%d_down_cm10_order5',...
                floor(tDRange{j}),10*xd(1),10*xd(2));
                ExitFlag = 0; %colChoice = 1;
            end
         else
             try
                 load(sprintf('./Data/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDCReaching_tSpan_%d_5_target_%d_%d_down_order5.mat',... 
                 floor(tDRange{j}),10*xd(1),10*xd(2)),...
                 'result','WOpt' ,'WIni', 'redThreshold','cm', 'tspan', 'mechSystem',...
                 'mechOpt', 'basisFunc', 'basisOpt','ExitFlag');
             catch
...
figure(1);
l = legend('3.0s','3.0s','3.0s','2.5s', '2.5s','2.5s','location','NorthWest');
set(l,'FontSize',14);
set(gca,'FontSize',14);
...
if(compWith == 1)
    figFolder = './Plots/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDC_MinAccl_';
else
    figFolder = './Plots/SynergyPaper/KC2lPassiveCompliant_LegendreBasisMDC/MDC_MinJerk_';
end
